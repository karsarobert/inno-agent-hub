...
description: >-
  Profi infografika generátor: 21 elrendezés × 21 vizuális stílussal, először elemzi a tartalmat és ajánl elrendezés×stílus kombinációt, majd közzétételre kész infografikát állít elő. Akkor használd, ha adatokat vagy tényeket infografikává szeretnél tenni. Trigger: infografika, infographic, vizuális poszter, diagram; visual summary.
  Profi infografika generátor: 21 elrendezés × 21 vizuális stílussal, először elemzi a tartalmat és ajánl elrendezés×stílus kombinációt, majd közzétételre kész infografikát állít elő. Akkor használd, ha adatokat vagy tényeket infografikává szeretnél tenni. Trigger: infografika, infographic, vizuális poszter, diagram; visual summary.
---
...
- Skip confirmation only when the current request explicitly says to do so, for example: `--no-confirm`, "generate directly", "skip confirmation", or equivalent wording.
...
|--------------|--------|--------------------|----------------|--------------|
| sűrű információs diagram / high-density-info | `dense-modules` | `morandi-journal`, `pop-laboratory`, `retro-pop-grid` | portrait | — |
| infografika / infographic | `bento-grid` | `craft-handmade` | landscape | Minimalist: clean canvas, ample whitespace, no complex background textures. Simple cartoon elements and icons only. |
...
- **Reconfigure interactively** — delete EXTEND.md (or ask "reconfigure baoyu-infographic preferences" / "reconfigure"). The next run re-triggers first-time setup.
