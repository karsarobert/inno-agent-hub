...
description: >-
  Professzionális dia-képek generálása tartalomból: először egy stílusleírással ellátott vázlatot készít, majd oldalanként legenerálja a diákat (a kimenet kép, ellentétben a szerkeszthető pptx-szel). Akkor használd, ha gyorsan kell egy prezentáció. Trigger: diák, prezentáció, slide deck, presentation, generate deck.
  Professzionális dia-képek generálása tartalomból: először egy stílusleírással ellátott vázlatot készít, majd oldalanként legenerálja a diákat (a kimenet kép, ellentétben a szerkeszthető pptx-szel). Akkor használd, ha gyorsan kell egy prezentáció. Trigger: diák, prezentáció, slide deck, presentation, generate deck.
---
...
- Skip confirmation only when the current request explicitly says to do so, for example: "generate directly", "skip confirmation", or equivalent wording.
...
**Hard gate**: this step is mandatory per the [Confirmation Policy](#confirmation-policy) — Steps 3+ cannot start until the user confirms here (or explicitly opts out with "generate directly" / equivalent wording in the current request).
